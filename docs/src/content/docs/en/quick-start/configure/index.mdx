...
Homebox applies configuration in this order: built-in defaults, an optional
YAML file, `HBOX_` environment variables, then command-line flags. Later
sources override earlier ones.

Pass the YAML path as a positional argument:

```bash
homebox /path/to/config.yml
```

The container images already pass `/data/config.yml`. That specific path is
optional so a new data volume can start without a file; when present, it is
loaded. An explicitly supplied path elsewhere must exist and contain valid
YAML. For example:
...
Keep secrets in a protected `.env` or secret store where possible. In
particular, `HBOX_AUTH_API_KEY_PEPPER` should remain stable across container
replacements and overrides any value in the YAML file.
...
| HBOX_OPTIONS_GITHUB_RELEASE_CHECK       | false                                                                                          | opt in to checking `nstefanelli/homebox` for a newer fork release. This remains off by default until the fork publishes releases.                                                         |
...
| HBOX_OPTIONS_HOSTNAME                   |                                                                                                | override the public origin used for OIDC redirects and other absolute URLs. Accepts a bare host (including an optional port or bracketed IPv6) or an absolute `http://`/`https://` origin; credentials, paths, queries, and fragments are rejected. |
...
| HBOX_BARCODE_OPEN_FOOD_FACTS_CONTACT    |                                                                                                | Optional contact value included in the User-Agent for Open Food Facts, Open Beauty Facts, and Open Products Facts barcode lookups and keyword searches. This is sent to third-party APIs, so avoid secrets or personal PII and prefer a role/contact alias. |
...
| HBOX_OTEL_PROXY_ENABLED                 | true                                                                                           | enable the in-process telemetry proxy endpoint that forwards frontend trace data to the configured exporter                                                                               |
    ...
    Product data returned by barcode lookups and keyword searches may come from [Open Food Facts](https://world.openfoodfacts.org), [Open Products Facts](https://world.openproductsfacts.org), and [Open Beauty Facts](https://world.openbeautyfacts.org), made available under the [Open Database License (ODbL)](https://opendatacommons.org/licenses/odbl/1-0/).
...
The example below starts Homebox in an isolated network. The process cannot
make external requests, including an explicitly enabled fork-release check.
