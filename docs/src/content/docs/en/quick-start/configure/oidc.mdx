...
2. **Provider Configuration**: Set the required provider details:
   ...
   - If using a reverse proxy, you may need to set `HBOX_OPTIONS_TRUST_PROXY=true` so Homebox can use the first `X-Forwarded-Host` and `X-Forwarded-Proto` values supplied by that trusted proxy.
   - Set `HBOX_OPTIONS_HOSTNAME` to either a bare public host (for example, `homebox.example.com`, `homebox.example.com:8443`, or `[2001:db8::1]:8443`) or an absolute `http://` or `https://` origin (for example, `https://homebox.example.com`). Do not include credentials, a path, a query string, or a fragment. An absolute origin is recommended when you want the public scheme to be explicit.
